...
export default function HomeScreen() {
  ...
  const [extraTerm, setExtraTerm] = React.useState(false);
  const [previous2Term, setPrevious2Term] = React.useState(false);
  const [previous3Term, setPrevious3Term] = React.useState(false);
  ...
  const previous2TermOptions = [
    { label: "Term 1", value: 1 },
    { label: "Term 2", value: 2 },
  ];
  const previous3TermOptions = [
    { label: "Term 1", value: 1 },
    { label: "Term 2", value: 2 },
    { label: "Term 3", value: 3 },
  ];
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      if (
        userInfo?.class &&
        user?.learner &&
        user?.learner?.class !== userInfo?.class
      ) {
        logoutHandler();
      }
      if (
        user?.learner?.schoolID.isExtraTerm === true &&
        (user?.learner?.class === "SSS 3" ||
          user?.learner?.class === "JSS 3" ||
          user?.learner?.class === "Grade 6" ||
          user?.learner?.class === "Grade 5") &&
        (currentTerm === 2 || currentTerm === 3)
      ) {
        setExtraTerm(true);
      } else {
        setExtraTerm(false);
      ...
      if (
        user?.learner?.schoolID.ispreviousTerm === true &&
        (user?.learner?.class !== "SSS 3" ||
          user?.learner?.class !== "JSS 3" ||
          user?.learner?.class !== "Grade 6" ||
          user?.learner?.class !== "Grade 5") &&
        ...
        setPrevious2Term(true);
      } else {
        setPrevious2Term(false);
      }
      if (
        user?.learner?.schoolID.ispreviousTerm === true &&
        user?.learner?.class != "SSS 3" &&
        ...
        setPrevious3Term(true);
      } else {
        setPrevious3Term(false);
      }
      refetch();
      refetchUser();
    }, [user, term])
          ...
          {extraTerm ? (
            <View className="  mb-1  bg-white h-12 rounded-lg shadow">
              <RNPickerSelect
                ...
                items={termOptions}
                placeholder={{ label: "Select previous Term", value: null }}
              />
            </View>
          ) : null}
          {previous2Term ? (
