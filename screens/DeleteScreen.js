...
const DeleteScreen = () => {
  ...
  const extractWordsInParentheses = (str) => {
    ...
    return matches && matches.length > 1 ? matches[1] : "pdf / video";
  };
    ...
    >
      {savedItems.length === 0 ? (
        ...
        <View className="w-full mt-4 mb-6">
          {savedItems?.map((item, index) => (
            <View key={item._id || index} className=" mx-2 mt-4">
              <Pressable
      ...
      {savedItems.length > 0 && (
        <Pressable
          ...
          className="py-3 bg-red-400 px-2 mb-8 rounded-xl w-full  max-h-[58px] flex items-center justify-center"
        >
